...
int	ft_unset(t_table *table, char **full_cmd, t_shell *shell)
{
	...
	while (full_cmd[++i])
	{
		if (ft_getenv(full_cmd[i], shell->env) == NULL && ft_getenv(full_cmd[i], shell->exported) != NULL)
			shell->exported = del_env(shell->exported, full_cmd[i]);
		else if(ft_getenv(full_cmd[i], shell->env) == NULL && ft_getenv(full_cmd[i], shell->exported) == NULL)
			continue;
		else
		{
